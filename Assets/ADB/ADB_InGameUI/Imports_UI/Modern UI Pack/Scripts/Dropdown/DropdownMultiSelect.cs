...
namespace Michsky.UI.ModernUIPack
{
    public class DropdownMultiSelect : MonoBehaviour, IPointerExitHandler
    {
        ...
        void ProcessModularAnimation()
        {
            if (isOn == true)
            {
                listCG.alpha += Time.deltaTime * transitionSmoothness;
                listRect.sizeDelta = Vector2.Lerp(listRect.sizeDelta, new Vector2(listRect.sizeDelta.x, panelSize), Time.deltaTime * sizeSmoothness);
            ...
            {
                listCG.alpha -= Time.deltaTime * transitionSmoothness;
                listRect.sizeDelta = Vector2.Lerp(listRect.sizeDelta, new Vector2(listRect.sizeDelta.x, closeOn), Time.deltaTime * sizeSmoothness);
